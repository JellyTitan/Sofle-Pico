...
- [ ] Automatic 'handedness'. There are no free spots in the matrix to use the matrix designation, and pulling one of the free pins high/low would require a 4.7k resistor, and I would rather not complicate the build with additional parts for something that will only get used once. (By 'once', I mean the alternative is to flash left/right hands separately. Presumably after the initial flash, all changes/updates would be done using VIA).
