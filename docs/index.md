...
> The VIA PR is in draft. The documentation is still in progress. You can see the punch list on the [status](/status) page.
...
The Sofle Pico support in VIA is in progress. Until then, the [via config can be manually uploaded](/flashing/via) using the web interface.
